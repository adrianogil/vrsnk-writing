...
O \textit{VRSnake} é a versão em realidade virtual do clássico jogo 2D \textit{Snake} que faz uso da unidade gráfica de processamento para calcular e renderizar um número expressivamente maior de oponentes enquanto jogos \textit{CPU-bound} processam uma quantidade menor. 

Originalmente controlava-se a serpente na busca pelos coletáveis espalhados pelo cenário, o \textit{VRSnake} permite ao jogador controlar os objetos coletáveis tornando seu principal objetivo derrotar as diversas serpentes que estão espalhadas pelo cenário. Propomos então as regras como seguem:

As serpentes buscam continuamente o objeto coletável colocado pelo jogador à medida que buscam o caminho que as mantenham vivas por mais tempo, isto é, as serpentes desviam de si mesmas e das demais serpentes quando possível, considerando um fator de randomização no processo de movimentação.
