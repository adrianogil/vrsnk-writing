...
\title{MR.Snake: Implementação baseada em Shaders de um Jogo de Realidade Mista}
...
% TODO: Refrasear texto
O \textit{MR. Snake} traz para a realidade virtual o clássico jogo 2D \textit{Snake}, mas diferentemente do original, o jogador assume controle sobre o posicionamento dos objetos coletáveis ao invés de controlar diretamente a serpente. Propomos então as seguintes regras: 
...
\section{MR. Snake: Um agente inteligente}
...
% TODO: Refrasear texto
O primeiro passo uso de \textit{Cubemap} é a geração de um cubo. Porém o cubo padrão gerado pela \textit{Unity} não possui vértices suficientes para um mapeamento de UV preciso. Para obter melhores resultados, utilizamos uma subdivisão de \textit{Mesh} em que cada triângulo é dividido em quatro. Iniciando de um cubo com 10 vértices e 12 triângulos, repetimos a operação de subdivisão quatro vezes e obtivemos um cubo de 4090 vértices e 4096 triângulos.

%% Calculo de UV
% TODO: Refrasear texto
No momento da geração de cada vértice, pode-se calcular a sua respectiva coordenada de UV, utilizando o cálculo da equação \ref{eq:skybox_equirect}. Percebe-se que assim, a visão cubemap equivale a discretização da abordagem de mapeamento contínuo das coordenadas de UV em um \textit{Skybox}.
