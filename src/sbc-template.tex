...
% Resumo proposta
Neste trabalho, propomos o desenvolvimento de um jogo cujas representações gráficas sejam inteiramente fundamentadas em código de GPU ao passo que as restrições lógicas concentrem-se na CPU, de maneira a atuar nos diferentes estágios da pipeline gráfica. Em alusão à um clássico, o jogo Snake será recriado para os dispositivos de realidade virtual da Samsung em uma aplicação Unity, no entanto, diferencia-se do original à medida que o jogador posiciona o objeto coletável através do controlador tendo a finalidade de fazer a serpente persegui-lo. Neste aspecto, aplicamos o Filtro de Kalman durante a leitura do posicionamento do cursor deste \textit{joystick} a fim de melhorar o reconhecimento do sinal e atenuar a inconsistência apresentada.
...
% GPGPU games
O jogo bidimensional Jelly in the Sky, recentemente disponibilizado para aquisição na plataforma de jogos para computador Steam, utiliza uma estrutura baseada em shader onde tanto os elementos visuais quanto a lógica de jogo são processados na GPU. No artigo \cite{joselli_clua_2009} é defendido um trabalho onde a GPU executa todo o processamento do jogo criado.

\section{VRSnake} \label{sec:vrsnake}
% Definição das regras do jogo
Neste artigo descrevemos o \textit{VRSnake}, um jogo de realidade virtual que apresenta uma modificação das regras do clássico jogo 2D \textit{Snake}:

\begin{enumerate}
  \item o jogador assume controle sobre o posicionamento dos objetos coletáveis ao invés de controlar diretamente a serpente
  \item A serpente busca contínua e automaticamente o objeto coletável e cresce em uma unidade à medida que alcança este objeto.
  \item Como no jogo original, a serpente deve movimentar-se de maneira circular pelo cenário, isto é, ao encontrar os limites do cenário de um lado, deve continuar do outro na mesma maneira.
  \item O jogador vence quando a \textit{snake} é derrotada, ou seja, quando ela atinge a si própria de alguma forma.
  \item Para garantir a diversão do jogo, o comportamento de busca da serpente deve garantir que não haja movimentação em direção a seu próprio corpo, além de utilizar de algum fator de aleatoriedade durante o movimento.
\end{enumerate}
...
A distância entre dois pontos em um espaço vetorial é dada pela magnitude da diferença de um vetor pelo outro, equação também conhecida como distância euclidiana, entretanto, considerando a restrição de deslocamento na qual a serpente pode-se mover unicamente nas direções apontadas pela Figura \ref{fig:snakeHeadPositions}, a distância euclidiana, mostrada na equação \ref{equation:euclidian}, não é vantajosa, pois envolve operações quadráticas as quais podem ser chamadas inúmeras vezes por segundo devido ao escopo desta aplicação, desta maneira o cálculo por meio da distância de Manhattan, equação \ref{equation:manhattan}, se mostra menos custoso.
...
Para a efetiva implementação do Filtro de Kalman na leitura do controle de realidade virtual seguem-se os seguintes passos: inicialmente captura-se a orientação angular do joystick, traça-se um raio da posição virtual atual do controle até uma esfera unitária e o ponto resultante da interseção é o foco do usuário, em outras palavras, o objetivo apontado pelo cursor, esta posição é onde aplica-se o Filtro de Kalman, proveniente da implementação encontrada na aplicação de \cite{IBUKURO}. O diagrama abaixo ilustra este processo.
...
Desenvolveu-se em realidade virtual o jogo \textit{Snake} no ambiente de desenvolvimento \textit{Unity}. Gerou-se uma aplicação \textit{android} testada no \textit{Samsung Galaxy S8} através do \textit{GearVR} com o controle de modelo ET-YO32, conforme apresentado na figura \ref{fig:vrcontroller}. Trabalhos de temática similar, como visto em \cite{joselli_clua_2009}, utilizam das taxas de quadros-por-segundo para mensurar a performance do jogo, haja vista que a quantidade de elementos renderizados durante a execução afeta diretamente esta métrica. A figura \ref{fig:VRPerformanceChart} abaixo ilustra a taxa de quadros no \textit{GearVR} coletados durante a execução da aplicação através da ferramenta de avaliação de performance da \textit{Oculus}, o \textit{OVR Metrics Tool}:
...
A aplicação apresentou taxa média de 43.96 \textit{fps} (\textit{frames-per-second}, quadros-por-segundo), com mínima de 16 \textit{fps} e máxima de 60 \textit{fps}, números adequados para um jogo, consoante \cite{joselli_montenegro_2009}.  Conforme representado pela figura \ref{fig:VRPerformanceChart}, a aplicação não mantém estavéis 60 \textit{fps} e isto se deve principalmente ao \textit{garbage collector} e à implementação do filtro de Kalman que, apesar de minimizar as inconsistências de leitura, ocasiona pequenos atrasos de processamento.

Provou-se também que é possível a execução de um jogo de realidade virtual com a renderização de seus elementos gráficos diretamente na unidade gráfica de processamento, através dos \textit{shaders}. Isto possibilita considerar o emprego desta arquitetura parcialmente ou completamente, viabilizando o uso da unidade central de processamento em outras atividades.

\section{Conclusão}\label{sec:conclusion}

% Resumo do artigo - Arquitetura
Apresentou-se a implementação de um jogo criado segundo a arquitetura de renderização baseada em \textit{shaders} que distribui passos usualmente restritos à camada de aplicação para as demais camadas da \textit{pipeline} gráfica. Assim, uma implementação baseada em \textit{Fragment Shader} em um único \textit{Mesh} foi utilizada para a renderização de todos os elementos do jogo.

% Resumo do artigo - Kalman
A fim de melhorar a experiência de uso do jogo com o controle, aplicou-se o filtro de Kalman com a intenção de aperfeiçoar a precisão das leituras da rotação do controle. Testes iniciais indicam que a implementação do filtro de Kalman tornou o movimento do curso mais suave e trouxe uma maior sensação de controle por parte do usuário.

% Trabalhos Futuros
% - Camada lógica para a GPU
% - Realizar testes comparativos com diferentes métodos de filtragem para avaliar vantagens e desvantagens de cada método
Em trabalhos futuros, três passos são percebidos como cruciais. Em primeiro lugar, migrar inteiramente os processos da camada lógica para serem executados na unidade gráfica de processamento, com o propósito de permitir o controle e exibição de um número maior de serpentes, acrescentando um maior fator lúdico. Em segundo lugar, realizar testes comparativos com diferentes métodos de filtragem do sinal do controle VR para avaliar vantagens e desvantagens de cada método. Por último, otimizar e melhorar o desempenho da aplicação, encontrando pontos de gargalo, de forma a possibilitar um melhor desempenho em termos de \textit{fps}.
