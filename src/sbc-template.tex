...
Neste cenário, a \textit{Unity} situa-se como peça de extrema relevância dadas a sua facilidade de aprendizado, gratuidade do uso e interatividade entre a comunidade de criadores.
...
Há essencialmente duas camadas respectivamente responsáveis pela visualização (ou renderização) e gerenciamento lógico da aplicação. A primeira envolve um shader, código executado diretamente na unidade de processamento gráfico (GPU), incumbido de delinear (ou renderizar) todos os itens apresentados no dispositivo de saída, que incluem plano de fundo, o objeto coletável circular bem como os quadriláteros que representam o personagem ou a \textit{snake}. A segunda camada, executada na unidade central de processamento (CPU), é encarregada das tarefas mais inteligentes tais como a procura pelo objeto coletável e movimentação. Faz-se necessário, portanto, explorar o funcionamento das etapas desenvolvidas, mencionadas acima. 
...
\begin{tikzpicture} [x=1.2cm, y=1cm, node distance=0,outer sep=0,inner sep=0]
	...
	\draw (0.4, 0.2) rectangle (5.8, 0.7) node[pos=.5] (5A) {\small Cálculo da Geometria do Plano de Fundo};
	\draw (7, 0.2) rectangle (11.7, 0.7) node[pos=.5] (6A) {\small Renderização do Plano de Fundo};	
