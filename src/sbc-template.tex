...
% Resumo proposta
Neste trabalho, propomos o desenvolvimento de um jogo cujas representações gráficas sejam inteiramente fundamentadas em código de GPU ao passo que as restrições lógicas concentrem-se na CPU, de maneira a atuar nos diferentes estágios da \textit{pipeline} gráfica acima apresentada. Em alusão à um clássico, o jogo \textit{Snake} será recriado para os dispositivos de realidade virtual da Samsung em uma aplicação Unity, no entanto, diferencia-se do original à medida que o jogador posiciona o objeto coletável através do controlador tendo a finalidade de fazer a serpente persegui-lo. Neste aspecto, objetivando solucionar o primeiro desafio acima exposto intenta-se a aplicação do filtro de Kalman durante a leitura do posicionamento do cursor através deste \textit{joystick}, a fim de melhorar o reconhecimento do sinal e atenuar a inconsistência apresentada.
...
Considerando ainda a restrição de deslocamento na qual a serpente pode-se mover unicamente nas direções apontadas pela figura \ref{fig:snakeHeadPositions}, a equação Euclidiana mostrada na equação \ref{equation:euclidian}, usualmente utilizada para cálculo de distância, envolve operações quadráticas as quais podem ser chamadas inúmeras vezes por segundo devido ao escopo desta aplicação, desta maneira o cálculo por meio da distância de Manhattan, equação \ref{equation:manhattan}, se mostrou menos custoso.

\begin{equation}
DistanciaEuclidiana = \sqrt{(x_{1} - x_{2})^2 + (y_{1} - y_{2})^2}
\label{equation:euclidian}
\end{equation}

\begin{equation}
DistanciaDeManhattan = \left|x_{1} - x_{2}\right| + \left|y_{1} - y_{2}\right|
