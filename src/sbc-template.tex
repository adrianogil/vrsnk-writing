...
Aplicações VR se diferenciam de outras aplicações pela preocupação em preencher o conteúdo do espaço tridimensional de forma a fornecer um conteúdo para todos os pontos de vista possíveis. Como por exemplo, a aplicação descrita em \cite{zund2015unfolding}, utiliza visão computacional para geração de uma visão panorâmica de um jogo de console de 8-bits. Rodando em um Oculus Rift DK2, o jogador é posicionado no centro do mundo, e a medida que movimenta seu personagem, o mundo se desdobra ao seu redor, extendendo a visão do jogo nas quatro paredes do ambiente virtual.

% 3 - Arquiteturas de visualização

% 4 - Controles de VR auxiliam na sensação de presença e por consequência na imersão
Controles VR, tal como o modelo ET-YO32 da Samsung exibido na figura \ref{fig:vrcontroller}, são definidos como sendo de mapeamento natural tangível realístico, que segundo classificação de \cite{skalski2011mapping} significa que o controle emula de forma realista a sensação do que está sendo feito dentro do ambiente virtual, ou seja, movimentar a réplica do controle físico no espaço 3D da aplicação VR.
