...
No espectro de desenvolvimento de aplicações VR, o motor e editor gráfico \textit{Unity} situa-se como peça de extrema relevância dadas a sua facilidade de aprendizado e incomplexidade no processo de desenvolvimento, além da possibilidade a criação de jogos e aplicações para plataformas móveis, consoles e de realidade virtual e aumentada.
...
O diagrama abaixo ilustra, de maneira geral, o funcionamento da arquitetura. Fundamentalmente, a camada lógica gerencia a colisão e a movimentação da \textit{snake}; decide o caminho mais apropriado somado a um fator de randomização; aplica o filtro de Kalman às entradas do controle; além de calcular a geometria do plano de fundo. A camada de visualização, construída em \textit{shader}, é responsável por renderizar a serpente bem como todos os demais elementos apresentados no cenário, isto é, não existem elementos visuais sob controle da unidade central de processamento.
