...
A realidade virtual (VR, em inglês) tem angariado espaço mercadológico à medida que são disponibilizados novos \textit{head mounted displays} (HMDs) e dispositivos  capazes de executar aplicações VR, especialmente os dispositivos móveis. Desta maneira, novas oportunidades e desafios mostram-se inerentes à esta tecnologia.  Os controladores ou \textit{joysticks}, por exemplo, são passíveis de ruídos devido à sua natureza sensorial, o que origina comportamentos indesejados como \textit{jittering} durante a leitura dos dados.  
...
Normalmente, os jogos e aplicações desenvolvidos na Unity são compostos por conjuntos de cenas, cada qual com seu próprio agrupamento de objetos e lógica associados. Desta maneira, a CPU é responsável por transmitir informações sobre elementos gráficos à GPU. Este processo acontece em dois estágios: primeiro os \textit{assets} gráficos como modelos e texturas são carregados na VRAM (\textit{video RAM}), posteriormente a CPU diz à GPU o que fazer, à medida que descreve os diferentes objetos renderizados na tela, como seus respectivos posicionamentos e demais informes relevantes. Por seu lado, a GPU recolhe esta informação e a converte em imagem \cite{akenine2008real}.

% Resumo proposta
Desse modo, propõe-se o desenvolvimento de um jogo cujas representações gráficas sejam inteiramente fundamentadas em código de GPU ao passo que as restrições lógicas concentrem-se na CPU, de maneira a aliviar o trabalho desta última. Em alusão à um clássico, o jogo Snake, de 1979, será recriado para os dispositivos de realidade virtual da Samsung em uma aplicação Unity, diferencia-se, porém, na medida que o jogador controla o objeto coletável, posicionando-o com o controlador tendo a finalidade de fazer a serpente falhar em seu objetivo. Neste aspecto, intenta-se a aplicação do filtro de Kalman durante a leitura do posicionamento do cursor através deste \textit{joystick}, a fim de melhorar o reconhecimento do sinal e atenuar a inconsistência apresentada. 
