...
A ilusão de um mundo virtual e consequentemente a sensação de imersão, requer que haja conteúdo visual para todos os ângulos possíveis, dado que o gearVR possibilita completa liberdade de rotação, ou seja 3 graus de liberdade (3DoF - \textit{3 Degrees of Freedom}). Como o nosso jogo proposto é essencialmente em 2D, tal como o Snake original, temos então o desafio de exibir conteúdo bidimensional em um cenário 3D, de forma que o conteúdo aconteça ao redor do usuário.

Uma esfera invertida, ou seja, uma esfera que tenha apenas seu lado interno sendo renderizado, possibilita preencher completamente todo o campo de visão possível do usuário, além de ser uma solução adotada na exibição de imagens equiretangulares em 360. A geração procedural de uma esfera pode seguir uma das duas abordagens abaixo:
\begin{enumerate}
...
Neste trabalho, fizemos uso da segunda abordagem devido a possibilidade de usar a longitude/latitude como forma de mapear as coordenadas de UV. Então geramos proceduralmente um \textit{mesh} de esfera de raio unitário segundo o pseudocódigo abaixo:
...
Para calcular as posições dos vertíces da esfera, dada uma quantidade de valores de latitudes $N_{latitude}$ e uma quantidade de valores de longitude $N_{longitude}$, definimos um valor $R_{longitude}$ como o tamanho angular de um longitude de uma secção transversal da esfera, tal como visto na equação \ref{equation1}.
...
Nas equações \ref{equation3} e \ref{equation4} são definidas as posições X e Z de pontos da esfera pertencentes a uma secção transversal da esfera onde possui um raio $D$.
...
Em um corte longitudinal, é possível perceber que a o raio $D$ da secção transversal é variável ao longo da altura da esfera. Estabelecemos assim, um valor $R$ como sendo o tamanho angular de uma latitude da esfera, tal como visto na equação \ref{equation5}.
...
O tamanho angular total de uma quantidade de $i$ de valores de latitude pode ser dada pela equação \ref{equation6}.
...
A posição Y dos pontos da esfera pode ser dada pela equação \ref{equation7}.
\begin{equation}
y_{i} = radius * \cos(\alpha_{latitude})
\label{equation7}
\end{equation}
...
Aplicando a equação \ref{equation8} nas equações \ref{equation3} e \ref{equation4} obtemos as posições X e Z dos vertices da esfera em função de suas coordenadas de longitude e latitude.

\begin{equation}
x_{longitude} = 2 * \sin(\alpha_{latitude}) * \sin(\alpha_{longitude})
\label{equation4}
\end{equation}

\begin{equation}
z_{longitude} = 2 * \sin(\alpha_{latitude}) * \cos(\alpha_{longitude})
\label{equation4}
\end{equation}
